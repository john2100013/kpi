...
class MeetingsController extends BaseController {
  ...
  async getAll(req, res) {
    ...
    return this.success(res, { meetings: result.rows });
  }
  ...
  async getById(req, res) {
    ...
    if (result.rows.length === 0) {
      return this.notFound(res, 'Meeting not found');
    }

    return this.success(res, { meeting: result.rows[0] });
  }
  ...
  async create(req, res) {
    ...
    if (!employee_id || !meeting_type || !scheduled_date) {
      return this.validationError(res, 'employee_id, meeting_type, and scheduled_date are required');
    }

    if (!['kpi_setting', 'kpi_review'].includes(meeting_type)) {
      return this.validationError(res, 'meeting_type must be "kpi_setting" or "kpi_review"');
    }
    ...
    if (employeeCheck.rows.length === 0) {
      return this.forbidden(res, 'Employee not found or does not belong to your team');
    }
    ...
    if (kpi_id) {
      ...
      if (kpiCheck.rows.length === 0) {
        return this.notFound(res, 'KPI not found or does not belong to this employee');
      }
    }
    ...
    return this.success(res, { meeting }, 201);
  }
  ...
  async delete(req, res) {
    ...
    return this.success(res, { message: 'Meeting deleted successfully' });
  }
}
