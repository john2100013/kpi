...
class AuthController extends BaseController {
  /**
   * Login user with email+password or payroll+password
   */
  async login(req, res) {
    try {
      ...
      if (method === 'email') {
        ...
        if (!email || !password) {
          return this.validationError(res, 'Email and password are required');
        }
        ...
        if (!payrollNumber || !password) {
          return this.validationError(res, 'Payroll number and password are required');
        }

        result = await query('SELECT * FROM users WHERE payroll_number = $1', [payrollNumber]);

        if (result.rows.length === 0) {
          return this.unauthorized(res, 'Invalid payroll number or password');
        }
        ...
        return this.validationError(res, 'Invalid login method');
      }
      ...
      if (user.role !== 'super_admin') {
        ...
        if (userCompanies.length === 0) {
          return this.forbidden(res, 'User must be associated with at least one company');
        }
  ...
  async selectCompany(req, res) {
    try {
      ...
      if (result.rows.length === 0) {
        return this.forbidden(res, 'You do not have access to this company');
      }
      ...
      return this.success(res, { token, companyId: parseInt(companyId) });
    } catch (error) {
  ...
  async updateProfile(req, res) {
    try {
      ...
      if (!['hr', 'manager', 'super_admin'].includes(req.user.role)) {
        return this.forbidden(res, 'You do not have permission to edit your profile');
      }
      ...
      if (email !== undefined && email.trim()) {
        ...
        if (emailCheck.rows.length > 0) {
          return this.validationError(res, 'Email is already taken');
        }
      ...
      if (national_id !== undefined) {
        ...
        if (national_id && national_id.trim()) {
          ...
          if (nationalIdCheck.rows.length > 0) {
            return this.validationError(res, 'National ID is already taken');
          }
        }
      ...
      if (payroll_number !== undefined && payroll_number.trim()) {
        ...
        if (payrollCheck.rows.length > 0) {
          return this.validationError(res, 'Payroll number is already taken');
        }
  ...
  async changePassword(req, res) {
    try {
      ...
      // Validate input - oldPassword is optional for first-time password changes
      if (!newPassword || !confirmPassword) {
        return this.validationError(res, 'New password and confirm password are required');
      }

      // Check if new password and confirm password match
      if (newPassword !== confirmPassword) {
        return this.validationError(res, 'New password and confirm password do not match');
      }

      // Validate new password strength (minimum 6 characters)
      if (newPassword.length < 6) {
        return this.validationError(res, 'New password must be at least 6 characters long');
      }
      ...
      // If oldPassword is provided, verify it (for non-first-time changes)
      if (oldPassword) {
        if (!user.password_hash) {
          return this.validationError(res, 'No password set for this account. Please contact administrator.');
        }

        const validPassword = await bcrypt.compare(oldPassword, user.password_hash);
        if (!validPassword) {
          return this.unauthorized(res, 'Old password is incorrect');
        }

        // Check if new password is different from old password
        if (oldPassword === newPassword) {
          return this.validationError(res, 'New password must be different from old password');
        }
      }
